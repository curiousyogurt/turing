...
Start with a tape with the characters @tt{əə0_0}, in configuration @tt{begin}, and have the head start on square @tt{2}.  Print output for steps @tt{400} to @tt{500}.
...
Example 5 tells the machine to print @tt{0} followed by a single @tt{1}, then @tt{0} followed by two @tt{1}s, then a @tt{0} followed by three @tt{1}s, and so on.  The first line of the body sets up the tape by printing symbols on successive squares, resulting in @tt{əə0_0}.  Then the machine goes into configuration @tt{o}, and starts the main loop.
